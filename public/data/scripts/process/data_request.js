...
export async function sendEmail(receiver, dataset_id, motivation, applicationData, email) {
  ...
  const href = `http://0.0.0.0:1234/?process=data_request_response${encodeURIComponent("&")}request_id=${applicationData.filename}`
  console.log("href", decodeURIComponent(href))
  const body = "Data access request by " + encodeURIComponent(email) + ".\n See the dataset here https://demo.dataverse.org/dataset.xhtml?persistentId="+dataset_id +" .Click here to respond to the request: " + href
